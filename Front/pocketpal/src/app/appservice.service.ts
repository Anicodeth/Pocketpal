...
export class AppserviceService {
  ...
  signIn(email: string, password: string){
    ...
    return this.http.post("http://Pocket-pal-api.vercel.app/signin", body)
  }
  
  getProfile(token: string){
    const header = new HttpHeaders({'Authorization': `Bearer ${token}`});
    return this.http.get("http://Pocket-pal-api.vercel.app/profile",{headers: header})
  }
  ...
  getBudget(token: string, month: number, year: number, balance: string){
    ...
    return this.http.post("http://Pocket-pal-api.vercel.app/budgets", body, {headers: header})
  }

  deleteExpense(token: string, budget_index: any, expense_index: any){
    const header = new HttpHeaders({'Authorization': `Bearer ${token}`});
    return this.http.delete(`http://Pocket-pal-api.vercel.app/budget/${budget_index}/expenses/${expense_index}`, {headers: header})
  }
  
  deletBudget(token: string, budget_index: any){
    const header = new HttpHeaders({'Authorization': `Bearer ${token}`});
    return this.http.delete(`http://Pocket-pal-api.vercel.app/budget/${budget_index}`, {headers: header})
  }
  
  getAIprompt(prompt: string){
    return this.http.get(`http://Pocket-pal-api.vercel.app/${prompt}`)
  }
